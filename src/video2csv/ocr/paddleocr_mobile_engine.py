...
class PaddleOCRMobileEngine:
    """Full OCR pipeline using lightweight mobile models.

    Uses PP-OCRv4_mobile_det (4.7 MB) + en_PP-OCRv4_mobile_rec (6.8 MB)
    instead of the default server models (~165 MB combined).
    Keeps text detection for proper handling of signed values and Y-drift.
    """
    ...
    def image_to_string(self, image: NDArray[np.uint8]) -> str:
        ...
        logger.debug(
            "PaddleOCR mobile picked: %r (area=%.0f, score=%.3f) from %d candidates",
